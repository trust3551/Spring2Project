...
public class ValidationController {
	 ...
	 * 		1. 입력값 검증
	 * 
	 * 		- 스프링 MVC Bean Validation 기능을 이용해 파라미터 값이 바인딩된 도메인 클래스(또는 커멘드 클래스)의 입력값 검증을 한다.
	 * 
	 * 			[환경설정]
	 * 			# 의존 관계 정의
	 ...
	 * 				> @Validated 를 지정한다.
	 * 				> 입력값 검증 대상의 도메인 클래스 직후에 BindinfResult를 정의한다.
	 * 					: BindingResult에는 요청 데이터의 바인딩 오류와 입력값 검증 오류 정보가 저장된다.
	 * 
	 * 			# 입력값 검증 환경설정 순서
	 * 			1. 입력값 검증을 위한 의존 라이브러리를 추가
	 * 			2. 입력값 검증 활성화
	 * 				> 활성화 할 도메인 클래스안에 '@xxx'와 같은 어노테이션으로 규칙 활성화
	 * 				> 데이터를 받을 컨트롤러 메소드 파라미터 영역의 선언된 도메인 클래스 앞에 @Validated를 붙여 검증 활성화
	 * 			3. 도메인 클래스 내 필드에다가 검증을 위한 어노테이션들로 데이터 검증을 설정한다. (@NotBlank, @Size 등등)
	 * 			4. 에러를 받을 BindingResult를 설정한다. (컨트롤러 메소드 내 설정)
	 */
	...
	public String registerValidationResult(@Validated Member member, BindingResult result, Model model) {
		log.info("registerValidationResult() 실행...!");
	...
	public String registerValidationResult2(@Validated Member member, BindingResult result, Model model) {
	 ...
	 *  		> userId, password, userName, email, dataOfBirth
	 *  	- 테스트는 registerValidationForm02에서 진행
	 * 
	 * 	4. 중첩된 자바빈즈 입력값 검증
	 *		- 중첩된 자바빈즈와 자바빈즈의 컬렉션에서 정의한 프로퍼티에 대해 입력값 검증을 할 떄는 @Valid를 지정한다.
	 *
	...
	public String registerValidationRsult3(@Validated Member member, BindingResult result) {
		if(result.hasErrors()) {
			return "validation/registerValidationForm03";
		}
