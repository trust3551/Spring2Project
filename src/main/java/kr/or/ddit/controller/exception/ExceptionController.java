...
public class ExceptionController {
	 ...
	 * 			- C. 존재하지 않는 게시물을 조회할때, 사용자가 정의한 사용자 예외발생
	 ...
	 * 			B. 삭제 할 때, 매핑 파일에서 예외 발생
	 * 				- Http Status 500
	 * 				- sqlmap/boardMapper_SQL.xml 내 id가 delete인 삭제기능을 가진 태그에서 board_no를 board_no2로 수정
	 * 
	 * 			C. 존재하지 않는 게시물을 조회할 때, 사용자가 정의한 예외 발생
	 * 				- Http Status 500
	 * 				- kr.or.ddit.service.impl.BoardServiceImpl read() 수정
	 * 					new BoardRecordNotFoundException() 예외발생
	 * 
	 * 			D. 존재하지 않는 페이지 URL 요청 시 예외 발생
	 * 				- Http Status 404
	 * 				- /crud/board/retry
	 ...
	 * 		- @ExceptionHandle 어노테이션은 괄호안에 설정한 예외 타입을 해당 메서드가 처리한다는 것을 의미한다.
	 * 
	 * 		예외 처리 핸들러 생성
	 ...
	 * 
	 * 	7. 예외 정보 출력
	 * 	- 예외에 대한 내용을 Model 객체를 이용해서 전달하고 뷰 화면에서 출력이 가능하다.
	 * 
	 ...
	 * 		예외 처리 방법
	 * 		- 웹 컨테이너 설정
	 * 			> 404에러를 처리할 수 있도록 DispatcherServlet의 throwExceptionIfNoHandlerFound 속성 true로 설정
	 * 
	 * 		- 처리할 수 있는 예외
	 * 			> 존재하지 않는 페이지 URL 요청시 예외 발생
	 * 		- 처리할 수 없는 예외
	 ...
	 * 			> BindingResult 에는 요청 데이터의 바인딩 에러와 검사 에러 정보가 저장된다.
	 * 
	 * 
	 */
}
