...
public class JSTLHomeController {
	 ...
	 *  		이런 중복되는 노력을 없애기 위해서 나온 것이 바로 JSTL 이다.
	 *  
	 *  	1) core 태그 라이브러리 
	 *  
	 *  		요소 		 │		설명
	 *  	────────────────────────────────────────────────────────────────────
	 *  	<c:out>		 │ JSPWriter에 값을 적절하게 처리한 후 에 출력한다.
	 ...
	 *  	<c:forToekns>│ 구분자로 구분된 각각의 토큰을 처리할 때 사용한다.
	 ...
	 *		<fn:imdexOf>			│ 지정한 문자열이 처음으로 나왔을 때의 인덱스를 구한다.
	 ...
	 *			scope		│	Stirng		│ 변수를 생성할 영역, 기본값은 page
	 ...
	 *
	 *		5) <c:if>
	 *			- 조건을 지정하고 지정된 조건과 일치하는 처리 내용을 구한다.
	 *
	 *			속성			│	타입			│		설명
	 *		──────────────────────────────────────────────────────────────
	 *			test		│	boolean		│ 검사 조건
	 ...
	 *
	 *		6) <c:choose>
	 *			- 여러 조건을 처리할 때 사용한다. (<c:when>, <c:otherwise>)
	 *
	 *		7) <c:when>
	 ...
	 *			cjarEncoding│	String		│ 결과를 읽어올 때 사용할 캐릭터 인코딩
	 *		──────────────────────────────────────────────────────────────
	 *
	 *		12) <c:url>
	 *			- URL을 재작성한다.
	 ...
	 *			scope		│	String		│ 변수를 저장할 영역
	 *		──────────────────────────────────────────────────────────────
	 *		value 속성값의 두가지 타임(절대 URL과 상대 URL)
	 *			- 절대 URL : 완전한 URL 이다.
	 *			- 상대 URL : 웹 애플리케이션 내에서의 절대 경로 :'/'로 시작하는 경로 (ex_/board/list.jsp)
	...
	// c:out escapeXml 속성의 기본값을 true이다.
	// c:out default 속성 null 값을 대체한다.
	@RequestMapping(value="/home0102", method = RequestMethod.GET)
	public String home0102(Model model) {
		log.info("home0102() 실행 ...!");
		Member member = new Member();
		member.setUserId("<p>hongkd#$%^</p>");	// escapeXml 테스트 위한 값 셋팅
		member.setPassword(null);				// default 테스트 위해 기존 값에 null 셋팅
	...
	// c:catch
	// EL안에서 발생하는 에러 정보는 EL안에서 처리하므로 var 속성에 설정된 변수로 에러정보를 확인할수 없습니다.
